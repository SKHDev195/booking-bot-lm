...
// #sb_client_phone is the real form field, but an intl-tel-input widget
// hides it (width:0/height:0) and injects a visible proxy input with a
// randomly-generated id (different every page load) and class
// "iti__tel-input" — that's the one a user actually types into, and its
// value only gets synced back into #sb_client_phone (the one that's
// actually submitted) on blur, not on every keystroke. Confirmed against
// the live page. Keep #sb_client_phone as a fallback in case some other
// flow/page variant doesn't have the widget at all.
const CLIENT_PHONE_SELECTORS = [".form-client-phone .iti__tel-input", "#sb_client_phone"];
...
async function fillIfPresent(page, selectors, value, { blurAfter = false } = {}) {
  for (const sel of selectors) {
    try {
      const el = page.locator(sel).first();
      if (await el.isVisible({ timeout: 2000 })) {
        await el.fill(value);
        // Some fields (e.g. the intl-tel-input phone widget) only sync their
        // displayed value into the actual form field that gets submitted on
        // blur, not on every keystroke/fill — confirmed against the live
        // page. A plain fill() alone leaves the real submitted value stale.
        if (blurAfter) await el.blur().catch(() => {});
        return true;
      }
    } catch (e) {
      // try next selector
...
async function fillClientDetails(page) {
  ...
  const filledPhone = await fillIfPresent(page, CLIENT_PHONE_SELECTORS, phone, { blurAfter: true });
  log(
...
async function completeBookingFlow(page) {
  ...
  if (!filled) {
    // fillClientDetails() already logged which of name/email/phone specifically
    // failed (see "Client form fields filled" line above this one).
    log("  Could not fill all required client detail fields — see field results above.");
    return false;
  }
