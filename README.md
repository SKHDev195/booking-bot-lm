...
every `burstIntervalSeconds` (20s) from `burstWindowMinutes` (2min) before to
`burstWindowMinutes` after each clean 30-minute mark (:00 / :30) — a 4-minute
window centered on the mark — then goes quiet until the next one's window.
...
This runs an immediate check (if it's within a burst window), then follows
the schedule in `config.js`: active 08:00–18:30 Asia/Nicosia, checking every
20s from 2 minutes before to 2 minutes after each clean :00/:30 mark, silent
otherwise and silent 00:00–08:00. Leave this process running.
...
- Check schedule: active 08:00–18:30 Asia/Nicosia, every 20s from 2min before
  to 2min after each clean 30-min mark (silent 00:00–08:00 and between marks)
